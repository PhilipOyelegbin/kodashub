...
function AdminWebsitePage() {
  ...
  const handleDeleteWebsite = async (id) => {
    try {
      const deleteWebsite = await fetch(`/api/development/${id}`, {
        method: "DELETE",
      });
      if (deleteWebsite.ok) {
        ...
        toast.error(deleteWebsite?.statusText);
      }
    } catch (error) {
      toast.error(error);
    }
  ...
  return (
    <section className='p-3 bg-white rounded shadow-md'>
      <div className='flex justify-between items-center'>
        ...
        <Link href='/admin/panel/websites/new'>
        ...
        {data?.map((item) => (
          <li className='rounded-md bg-purple-100' key={item.id}>
            ...
            <div className='flex gap-2 text-2xl py-2 px-4 border-t-2 text-right'>
              <Link href={`/admin/panel/websites/${item.id}`}>
